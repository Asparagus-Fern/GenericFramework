...
void SUMGSplineEditPanel::PaintGridLines(const FGeometry& AllottedGeometry, FSlateWindowElementList& OutDrawElements, int32 LayerId, const FSlateRect& MyCullingRect, ESlateDrawEffect DrawEffects) const
{
...
FText SUMGSplineEditPanel::GetPanelHeightLabel() const
{
	...
	return FText::AsNumber(UWidgetEdGenerationSetting::Get()->PanelHeight, &Options);
}

float SUMGSplineEditPanel::GetPanelHeightSliderPosition() const
{
	return (UWidgetEdGenerationSetting::Get()->PanelHeight - CONST_PanelHeightMin) / (CONST_PanelHeightMax - CONST_PanelHeightMin);
}
...
void SUMGSplineEditPanel::OnSetPanelHeight(float NewValue)
{
	...
	UWidgetEdGenerationSetting::Get()->PanelHeight = DesiredHeight;
}
