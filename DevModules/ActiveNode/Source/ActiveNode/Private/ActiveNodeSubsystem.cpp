...
void UActiveNodeSubsystem::Tick(float DeltaTime)
{
	...
	if(IsCurrentNodeActive() && !UActiveNodeSettings::Get()->bBlockUpdate)
	{
		...
		{
			ProcessingNodeInterfaceComponents(CurrentNode, [&DeltaTime](UActiveNodeComponent* InComponent)
			{
...
bool UActiveNodeSubsystem::ChangeNode(const FGameplayTag& InTag, bool bReInit)
{
	...
	ProcessingNodeInterfaceComponents(ChangeNode, [this](UActiveNodeComponent* Component)
	{
	...
	if(LoginNode(ChangeNode, bReInit))
	{
		...
		// 处理即将切换的节点所有组件的预切换接口事件.
		ProcessingNodeInterfaceComponents(ChangeNode, [](UActiveNodeComponent* Component)
		{
...
bool UActiveNodeSubsystem::LoginNode(AActiveNode* InNode, bool bReInit)
{
	...
	if(bReInit)
	{
		OnActiveNodePreChange.Broadcast(CurrentNode, CurrentNode, true);
	}
...
void UActiveNodeSubsystem::LoginOut(const UObject* WorldContextObject)
{
