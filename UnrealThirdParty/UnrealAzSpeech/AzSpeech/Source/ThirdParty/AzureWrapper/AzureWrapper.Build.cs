...
public class AzureWrapper : ModuleRules
{
	...
	{
		if (Target.Platform == UnrealTargetPlatform.Win64) return Path.Combine("libs", "Win");

		if (Target.Platform == UnrealTargetPlatform.Android) return Path.Combine("libs", "Android");

		if (Target.Platform == UnrealTargetPlatform.IOS) return Path.Combine("libs", "iOS");

		var ArchSubDir = isArmArch() ? "Arm64" : "x64";

		if (Target.Platform == UnrealTargetPlatform.Mac) return Path.Combine("libs", "Mac", ArchSubDir);

		if (Target.Platform.ToString().ToLower().Contains("linux")) return Path.Combine("libs", "Linux", ArchSubDir);
	...
	{
		if (Target.Platform == UnrealTargetPlatform.Win64) return Path.Combine(GetPlatformLibsSubDirectory(), "Runtime");
	...
	{
		if (!IsRuntimePlatform() || GetDynamicLibraries().Count <= 0) return;
		...
		else if (Target.Platform == UnrealTargetPlatform.Mac || Target.Platform.ToString().ToLower().Contains("linux")) PublicDefinitions.Add(string.Format("AZSPEECH_THIRDPARTY_BINARY_SUBDIR=\"{0}\"", GetRuntimesSubDirectory().Replace(@"\", @"/")));
	}
	...
	public AzureWrapper(ReadOnlyTargetRules Target) : base(Target)
	{
		...
		foreach (var StaticLib in GetStaticLibraries()) PublicAdditionalLibraries.Add(Path.Combine(GetPlatformLibsAbsoluteDirectory(), StaticLib));

		foreach (var DynamicLib in GetDynamicLibraries())
		{
		...
		else if (Target.Platform == UnrealTargetPlatform.Android) AdditionalPropertiesForReceipt.Add("AndroidPlugin", Path.Combine(ModuleDirectory, "AzSpeech_UPL_Android.xml"));
	}
}