...
public:
	...
	WIDGETAPPLICATION_API void UpdateDateTimeNow();

	UFUNCTION(BlueprintCallable)
	WIDGETAPPLICATION_API void SetIsUseHour12(bool IsUseHour12);
	...
	UFUNCTION(BlueprintNativeEvent, BlueprintPure)
	WIDGETAPPLICATION_API FText GetFormatTime(int32 InHour, int32 InMinute, int32 InSecond, int32 InMillisecond);

	UFUNCTION(BlueprintNativeEvent, BlueprintPure)
	WIDGETAPPLICATION_API FText GetFormatDayOfWeek(EDayInWeek DayInWeek);

	UFUNCTION(BlueprintNativeEvent, BlueprintPure)
	WIDGETAPPLICATION_API FText GetFormatMonthOfYear(EMonthInYear MonthInYear);

	UFUNCTION(BlueprintPure)
	WIDGETAPPLICATION_API bool GetIsMorning() const;

	UFUNCTION(BlueprintPure)
	WIDGETAPPLICATION_API bool GetIsAfternoon() const;

	UFUNCTION(BlueprintPure)
	WIDGETAPPLICATION_API bool GetIsLeapYear() const;
