...
TSharedRef<SWidget> SAzSpeechAudioGenerator::ConstructContent()
{
	...
	return SNew(SScrollBox)
		+ SScrollBox::Slot().Padding(MarginSpacing)
		[
			SNew(SGridPanel).FillColumn(1, 1.f)
			+ SGridPanel::Slot(0, 0).Padding(MarginSpacing)
			[
				SNew(STextBlock).Text(FText::FromString(TEXT("Synthesis Text/SSML"))).TextStyle(FAppStyle::Get(), "PropertyEditor.AssetClass").Font(
					FAppStyle::Get().GetFontStyle("PropertyWindow.NormalFont"))
			]
			+ SGridPanel::Slot(1, 0).Padding(MarginSpacing)
			[
			...
			+ SGridPanel::Slot(0, 1).Padding(MarginSpacing)
			[
				SNew(STextBlock).Text(FText::FromString(TEXT("Is SSML"))).TextStyle(FAppStyle::Get(), "PropertyEditor.AssetClass").Font(
					FAppStyle::Get().GetFontStyle("PropertyWindow.NormalFont"))
			]
			+ SGridPanel::Slot(1, 1).Padding(MarginSpacing)
			[
				SNew(SCheckBox).IsChecked(ECheckBoxState::Unchecked).OnCheckStateChanged_Lambda([this](const ECheckBoxState InState)
				{
					bIsSSMLBased = InState == ECheckBoxState::Checked;
				})
			]
			+ SGridPanel::Slot(0, 2).Padding(MarginSpacing)
			[
				SNew(STextBlock).Text(FText::FromString(TEXT("Locale"))).TextStyle(FAppStyle::Get(), "PropertyEditor.AssetClass").Font(
					FAppStyle::Get().GetFontStyle("PropertyWindow.NormalFont"))
			]
			+ SGridPanel::Slot(1, 2).Padding(MarginSpacing)
			[
				SNew(SEditableTextBox).OnTextCommitted_Lambda([this](const FText& InText, [[maybe_unused]] ETextCommit::Type InCommitType)
				{
			...
			+ SGridPanel::Slot(0, 3).Padding(MarginSpacing)
			[
				SNew(STextBlock).Text(FText::FromString(TEXT("Voice"))).TextStyle(FAppStyle::Get(), "PropertyEditor.AssetClass").Font(
					FAppStyle::Get().GetFontStyle("PropertyWindow.NormalFont"))
			]
			+ SGridPanel::Slot(1, 3).Padding(MarginSpacing)
			[
				SAssignNew(VoiceComboBox, STextComboBox).OptionsSource(&AvailableVoices).InitiallySelectedItem(SelectVoice).OnSelectionChanged_Lambda(
					[this](const TSharedPtr<FString>& InStr, [[maybe_unused]] ESelectInfo::Type)
					{
						Voice = InStr->TrimStartAndEnd();
					})
			]
			+ SGridPanel::Slot(0, 4).Padding(MarginSpacing)
			[
				SNew(STextBlock).Text(FText::FromString(TEXT("Module"))).TextStyle(FAppStyle::Get(), "PropertyEditor.AssetClass").Font(
					FAppStyle::Get().GetFontStyle("PropertyWindow.NormalFont"))
			]
			+ SGridPanel::Slot(1, 4).Padding(MarginSpacing)
			[
				SNew(STextComboBox).OptionsSource(&AvailableModules).InitiallySelectedItem(GameModule).OnSelectionChanged_Lambda(
					[this](const TSharedPtr<FString>& InStr, [[maybe_unused]] ESelectInfo::Type)
					{
						Module = InStr->TrimStartAndEnd();
					})
			]
			+ SGridPanel::Slot(0, 5).Padding(MarginSpacing)
			[
				SNew(STextBlock)
			...
			+ SGridPanel::Slot(1, 5).Padding(MarginSpacing)
			[
				SAssignNew(PathInput, SEditableTextBox).OnTextCommitted_Lambda(
					[this](const FText& InText, [[maybe_unused]] ETextCommit::Type InCommitType)
					{
						OnFileInfoCommited(InText, RelativePath, PathInput);
					})
			]
			+ SGridPanel::Slot(0, 6).Padding(MarginSpacing)
			[
				SNew(STextBlock).Text(FText::FromString(TEXT("Asset Name"))).TextStyle(FAppStyle::Get(), "PropertyEditor.AssetClass").Font(
					FAppStyle::Get().GetFontStyle("PropertyWindow.NormalFont"))
			]
			+ SGridPanel::Slot(1, 6).Padding(MarginSpacing)
			[
				SAssignNew(AssetNameInput, SEditableTextBox).OnTextCommitted_Lambda(
					[this](const FText& InText, [[maybe_unused]] ETextCommit::Type InCommitType)
					{
						OnFileInfoCommited(InText, AssetName, AssetNameInput);
					})
			]
			+ SGridPanel::Slot(1, 7).Padding(MarginSpacing).HAlign(HAlign_Left)
			[
				SNew(SButton)
...
void SAzSpeechAudioGenerator::UpdateAvailableVoices()
{
	...
	InternalGetter->OnAvailableVoicesUpdated.BindLambda([this](const TArray<FString>& Voices)
	{
...
FReply SAzSpeechAudioGenerator::HandleGenerateAudioButtonClicked()
{
	...
	InternalGetter->OnAudioDataGenerated.BindLambda([this](const TArray<uint8>& AudioData)
	{
