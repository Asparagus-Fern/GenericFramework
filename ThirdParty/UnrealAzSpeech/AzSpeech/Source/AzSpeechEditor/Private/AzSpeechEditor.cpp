...
void FAzSpeechEditorModule::RegisterMenus()
{
 ...
 const FName AppStyleName = FEditorStyle::GetStyleSetName();
	...
	const TSharedPtr<FWorkspaceItem> Menu = WorkspaceMenu::GetMenuStructure().GetToolsCategory()->AddGroup(
		LOCTEXT("AzSpeechCategory", "AzSpeech"), LOCTEXT("AzSpeechCategoryTooltip", "AzSpeech Plugin Tabs"),
	...
	FGlobalTabmanager::Get()->RegisterNomadTabSpawner(AzSpeechEditorTabName, EditorTabSpawnerDelegate).
	                          SetDisplayName(FText::FromString(TEXT("AzSpeech Audio Generator"))).SetTooltipText(
		                          FText::FromString(TEXT("Open AzSpeech Audio Generator"))).SetIcon(FSlateIcon(AppStyleName, "Icons.Plus")).SetGroup(
		                          Menu.ToSharedRef());
}
