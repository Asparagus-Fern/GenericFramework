...
public class FastNoise2 : ModuleRules
{
	...
	private string RuntimePath => Path.Combine("$(PluginDir)", RelativeRuntimePath);

	private string RelativeRuntimePath => Path.Combine("Binaries", "ThirdParty", "FastNoise2", PlatformString, LibraryName + RuntimeLibExtension);
	...
	private string LibraryName
	{
		get
		{
			if (Target.Configuration == UnrealTargetConfiguration.Debug)
			{
				...
				else if (Target.Platform.IsInGroup(UnrealPlatformGroup.Microsoft)) return "FastNoiseD";
			}
			else
			{
				if (Target.Platform == UnrealTargetPlatform.Mac ||
				    ...
				    Target.Platform == UnrealTargetPlatform.LinuxArm64)
					return "libFastNoise";
				else if (Target.Platform.IsInGroup(UnrealPlatformGroup.Microsoft)) return "FastNoise";
			}
		...
		get
		{
			if (Target.Platform == UnrealTargetPlatform.Mac)
				...
				return ".framework";
			         ...
			         Target.Platform == UnrealTargetPlatform.LinuxArm64)
				return ".so";
			else if (Target.Platform.IsInGroup(UnrealPlatformGroup.Microsoft)) return ".dll";
