...
DEFINE_FUNCTION(UBPFunctions_Property::execSetPropertyValue)
{
	...
	if (!TargetProperty->SameType(SourceProperty))
	{
		...
		GenericLOG(GenericLogDefault, Error, TEXT("TargetProperty Is Not Same As SourceProperty."))
		FBlueprintCoreDelegates::ThrowScriptException(P_THIS, Stack, ExceptionInfo);
		return;
	}
	...
	if (!SourceProperty || !SourceValuePtr)
	{
		...
		GenericLOG(GenericLogDefault, Error, TEXT("Failed to Resolve the Property For SetPropertyValue."))
		FBlueprintCoreDelegates::ThrowScriptException(P_THIS, Stack, ExceptionInfo);
		return;
	}
