 ...
 */
UCLASS()
class UNREALMISC_API UBPFunctions_Property : public UBlueprintFunctionLibrary
{
	GENERATED_BODY()
...
public:
	...
	static bool GetValueOfType(const UObject* Target, const FName& PropertyName, T& OutValue)
	{
		...
		{
			GenericLOG(GenericLogDefault, Error, TEXT("Find Null Property Named: %s"), *(PropertyName.ToString()));
			return false;
		}
