...
android.permission.BROADCAST_WAP_PUSHWAP                PUSH服务收到后触发一个广播
...
android.permission.ACCESS_SURFACE_FLINGERAndroid        平台上底层的图形显示支持，一般用于游戏或照相机预览界面和底层模式的屏幕截图
 ...
 */
UCLASS()
class UNREALMISC_API UAcquireAndroidPermissionAsyncAction : public UBlueprintAsyncActionBase
{
	GENERATED_BODY()
...
public:
	...
	static UAcquireAndroidPermissionAsyncAction* AsyncAcquireAndroidPermission(const FString& InPermission);

	UFUNCTION(BlueprintCallable, meta = (BlueprintInternalUseOnly = "true"))
	static UAcquireAndroidPermissionAsyncAction* AsyncAcquireAndroidPermissions(const TArray<FString>& InPermissions);
