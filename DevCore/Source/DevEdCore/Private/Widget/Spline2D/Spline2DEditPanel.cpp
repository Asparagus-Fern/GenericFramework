...
FSelectedSplineTangent::FSelectedSplineTangent()
{
}

bool FSelectedSplineTangent::IsValid() const
{
	return PointIndex != -1;
}
...
FSplineEditPanelTransform::FSplineEditPanelTransform()
{
	Offset = FVector2D::ZeroVector;
	Scale = 1.0;
}

FVector2D FSplineEditPanelTransform::LocalToInput(FVector2D Local) const
{
	return (Scale != 0.0f) ? (Local / Scale + Offset) : Offset;
}

FVector2D FSplineEditPanelTransform::InputToLocal(FVector2D Input) const
{
...
void SSpline2DEditPanel::Construct(const FArguments& InArgs)
{
	...
	[
				...
				[
					...
					[
						...
						[
							...
							.OnGetMenuContent(this, &SSpline2DEditPanel::GetPanelHeightMenu)
						...
						[
							...
							[
								SNew(SImage)
								.Image(FAppStyle::Get().GetBrush("UMGEditor.ZoomToFit"))
								.ColorAndOpacity(FSlateColor::UseForeground())
							]
						]
						...
						[
							SNew(SComboButton)
							.ButtonStyle(&ToolBarStyle.ButtonStyle)
							.OnGetMenuContent(this, &SSpline2DEditPanel::GetThicknessFillMenu)
							.ContentPadding(ToolBarStyle.ButtonPadding)
							.ButtonContent()
							[
								SNew(STextBlock)
		...
		[
			...
			.ToolTipText(this, &SSpline2DEditPanel::GetHelpText)
		]
	];
...
void SSpline2DEditPanel::Tick(const FGeometry& AllottedGeometry, const double InCurrentTime, const float InDeltaTime)
{
	// Zoom to extents
	if (bDeferredZoomToFit)
	{
		...
		RegisterActiveTimer(0.f, FWidgetActiveTimerDelegate::CreateSP(this, &SSpline2DEditPanel::HandleZoomToFit));
	}
}

FVector2D SSpline2DEditPanel::ComputeDesiredSize(float) const
{
	return FVector2D(300.0f, 310.0f);
}

FReply SSpline2DEditPanel::OnMouseButtonDown(const FGeometry& InMyGeometry, const FPointerEvent& InMouseEvent)
{
...
FReply SSpline2DEditPanel::OnMouseButtonUp(const FGeometry& InMyGeometry, const FPointerEvent& InMouseEvent)
{
	if (HasMouseCapture())
	{
...
FReply SSpline2DEditPanel::OnMouseMove(const FGeometry& InMyGeometry, const FPointerEvent& InMouseEvent)
{
...
void SSpline2DEditPanel::OnMouseCaptureLost(const FCaptureLostEvent& CaptureLostEvent)
{
	DragState = EDragState::None;
}

FReply SSpline2DEditPanel::OnMouseWheel(const FGeometry& MyGeometry, const FPointerEvent& MouseEvent)
{
	if (bEditPanelFocus)
	{
...
void SSpline2DEditPanel::OnMouseLeave(const FPointerEvent& MouseEvent)
{
	bEditPanelFocus = false;
}

FReply SSpline2DEditPanel::OnKeyDown(const FGeometry& MyGeometry, const FKeyEvent& InKeyEvent)
{
...
int32 SSpline2DEditPanel::OnPaint(const FPaintArgs& Args, const FGeometry& AllottedGeometry, const FSlateRect& MyCullingRect, FSlateWindowElementList& OutDrawElements, int32 LayerId, const FWidgetStyle& InWidgetStyle, bool bParentEnabled) const
{
	const ESlateDrawEffect DrawEffects = ShouldBeEnabled(bParentEnabled) ? ESlateDrawEffect::None : ESlateDrawEffect::DisabledEffect;

	const int32 BackgroundLayerId = LayerId;

	// grid lines.
	const int32 GridLineLayerId = BackgroundLayerId + 1;
	PaintGridLines(AllottedGeometry, OutDrawElements, GridLineLayerId, MyCullingRect, DrawEffects);

	const int32 SplineLayerId = GridLineLayerId + 1;
	...
	const int32 KeyLayerId = SplineLayerId + 1;
	const int32 SelectedKeyLayerId = KeyLayerId + 1;
	...
	// Paint children
	const int32 ChildrenLayerId = SelectedKeyLayerId + 1;
...
void SSpline2DEditPanel::PaintSpline_Default(const FSpline2DInfo& InSplineInfo, const FGeometry& AllottedGeometry, FSlateWindowElementList& OutDrawElements, int32 LayerId, const FSlateRect& MyCullingRect, ESlateDrawEffect DrawEffects, const FWidgetStyle& InWidgetStyle) const
{
...
void SSpline2DEditPanel::PaintSpline_CustomVerts(const FSpline2DInfo& InSplineInfo, const FGeometry& AllottedGeometry, FSlateWindowElementList& OutDrawElements, int32 LayerId, const FSlateRect& MyCullingRect, ESlateDrawEffect DrawEffects, const FWidgetStyle& InWidgetStyle) const
{
	if (InSplineInfo.GetPointNum() > 1)
	{
		const TArray<FSpline2DPoint>& Points = InSplineInfo.GetPoints();
...
void SSpline2DEditPanel::PaintSplinePoints(const FSpline2DInfo& InSplineInfo, FSlateWindowElementList& OutDrawElements, int32 LayerId, int32 SelectedLayerId, const FGeometry& AllottedGeometry, const FSlateRect& MyCullingRect, ESlateDrawEffect DrawEffects, const FWidgetStyle& InWidgetStyle) const
{
	...
	for (int32 i = 0; i < Points.Num(); ++i)
	{
		...
		// Get brush
		const bool IsSelected = IsSplinePointSelected(i);

		const int32 LayerToUse = IsSelected ? SelectedLayerId : LayerId;
...
void SSpline2DEditPanel::PaintTangent(const FSpline2DPoint& SplinePoint, FSlateWindowElementList& OutDrawElements, int32 LayerId, const FGeometry& AllottedGeometry, const FSlateRect& MyCullingRect, ESlateDrawEffect DrawEffects, int32 LayerToUse, const FWidgetStyle& InWidgetStyle, bool bTangentSelected) const
{
	...
	const FVector2D ArriveTangentIconLocation = ArriveTangentLocation - (CONST_TangentSize / 2);
	FVector2D LeaveTangentIconLocation = LeaveTangentLocation - (CONST_TangentSize / 2);

	const FSlateBrush* TangentBrush = FAppStyle::Get().GetBrush("CurveEd.Tangent");
	const FSlateBrush* TangentBrushSelected = FAppStyle::Get().GetBrush("CurveEd.TangentSelected");
	const FSlateBrush* ArrowImage = FAppStyle::Get().GetBrush(TEXT("Graph.Arrow"));
	const FLinearColor TangentColor = FAppStyle::Get().GetColor("CurveEd.TangentColor");
	const FLinearColor TangentColorSelected = FAppStyle::Get().GetColor("CurveEd.TangentColorSelected");

	const bool LeaveTangentSelected = bTangentSelected;
	const bool ArriveTangentSelected = bTangentSelected;
...
void SSpline2DEditPanel::PaintGridLines(const FGeometry& AllottedGeometry, FSlateWindowElementList& OutDrawElements, int32 LayerId, const FSlateRect& MyCullingRect, ESlateDrawEffect DrawEffects) const
{
	const float GridPixelSpacing = CalculateGridPixelSpacing();

	constexpr FLinearColor GridColor(0.35f, 0.35f, 0.35f, 0.2f);
	constexpr FLinearColor BoarderColor(0.35f, 0.35f, 0.35f, 1.0f);
	...
	//Vertical grid
	{
		...
		const float StartOffset = FMath::Fmod(-TransformInfo.Offset.X, GridPixelSpacing);
	...
	//Horizontal grid
	{
		...
		const float StartOffset = FMath::Fmod(-TransformInfo.Offset.Y, GridPixelSpacing);;
...
bool SSpline2DEditPanel::IsSplinePointSelected(int PointIndex) const
{
	return PointIndex == SelectedPointIndex;
}

void SSpline2DEditPanel::GetTangentPoints(const FVector2D& Location, const FVector2D& Direction, FVector2D& Arrive, FVector2D& Leave) const
{
	...
	const float KeyTangentOffset = FMath::Lerp(CONST_KeyTangentOffsetMin, CONST_KeyTangentOffsetMax, Alpha);
	Arrive = -ArriveDirection * KeyTangentOffset + Location;

	Leave = LeaveDirection * KeyTangentOffset + Location;
}

void SSpline2DEditPanel::TryStartDrag(const FGeometry& InMyGeometry, const FPointerEvent& InMouseEvent)
{
	...
	if (DragVector.SizeSquared() >= FMath::Square(DragThreshold))
	{
		if (bLeftMouseButton)
		{
			...
			const int HitPointIndex = HitTestSplinePoints(InMyGeometry, InMyGeometry.LocalToAbsolute(MouseDownLocation));
			if (HitPointIndex != -1)
			{
			...
			else
			{
				...
				const FSelectedSplineTangent HitTangent = HitTestCubicTangents(InMyGeometry, InMyGeometry.LocalToAbsolute(MouseDownLocation));
...
void SSpline2DEditPanel::ProcessDrag(const FGeometry& InMyGeometry, const FPointerEvent& InMouseEvent)
{
	...
	else if (DragState == EDragState::DragTangent)
	{
		...
		const FVector2D KeyLocation = SplineInfo.Get().Points[SelectedTangent.PointIndex].Location;
		...
		const float Strength = (CONST_TangentMaxStrength * Alpha) / TransformInfo.Scale;
...
void SSpline2DEditPanel::EndDrag(const FGeometry& InMyGeometry, const FPointerEvent& InMouseEvent)
{
	DragState = EDragState::None;
}

void SSpline2DEditPanel::ZoomView(const FGeometry& MyGeometry, const FPointerEvent& MouseEvent)
{
	...
	const FVector2D InPosition = TransformInfo.LocalToInput(MousePosition);
	const FVector2D NewOffset = InPosition - (MousePosition / NewScale);

	TransformInfo.Scale = NewScale;
	TransformInfo.Offset = NewOffset;
}

int SSpline2DEditPanel::HitTestSplinePoints(const FGeometry& InMyGeometry, const FVector2D& HitScreenPosition)
{
	...
	for (int32 i = 0; i < Points.Num(); ++i)
	{
		const FVector2D PointLocalLocation = TransformInfo.InputToLocal(Points[i].Location);
...
FSelectedSplineTangent SSpline2DEditPanel::HitTestCubicTangents(const FGeometry& InMyGeometry, const FVector2D& HitScreenPosition)
{
...
void SSpline2DEditPanel::ProcessClick(const FGeometry& InMyGeometry, const FPointerEvent& InMouseEvent)
{
	const bool bLeftMouseButton = InMouseEvent.GetEffectingButton() == EKeys::LeftMouseButton;
	const bool bRightMouseButton = InMouseEvent.GetEffectingButton() == EKeys::RightMouseButton;

	const int HitPointIndex = HitTestSplinePoints(InMyGeometry, InMouseEvent.GetScreenSpacePosition());
	const FSelectedSplineTangent HitTangent = HitTestCubicTangents(InMyGeometry, InMouseEvent.GetScreenSpacePosition());
...
void SSpline2DEditPanel::CreateContextMenu(const FGeometry& InMyGeometry, const FPointerEvent& InMouseEvent, bool bShowAddNewSplinePoint)
{
	...
	MenuBuilder.BeginSection("EditUMGSplineActions", LOCTEXT("Actions", "Actions"));
	{
		if (bShowAddNewSplinePoint)
		{
			...
			FUIAction Action = FUIAction(FExecuteAction::CreateSP(this, &SSpline2DEditPanel::AddNewSplinePoint, InMyGeometry, Position));
		...
		if (SelectedPointIndex != -1)
		{
			...
			FUIAction Action = FUIAction(FExecuteAction::CreateSP(this, &SSpline2DEditPanel::DeleteSelectedSplinePoint));
			MenuBuilder.AddMenuEntry(MenuItemLabel, MenuItemToolTip, FSlateIcon(), Action);
		}
	}
...
float SSpline2DEditPanel::CalculateGridPixelSpacing() const
{
...
void SSpline2DEditPanel::DeferredZoomToFit()
{
	bDeferredZoomToFit = true;
}

EActiveTimerReturnType SSpline2DEditPanel::HandleZoomToFit(double InCurrentTime, float InDeltaTime)
{
	if (ZoomToFit(true, true))
	{
...
bool SSpline2DEditPanel::ZoomToFit(bool bFitHorizontal, bool bFitVertical)
{
	...
	if (bFitHorizontal && bFitVertical)
	{
		...
		const FVector2D InputCenter = FVector2D(InMaxX + InMinX, InMaxY + InMinY) / 2;
	...
	{
		const float Scale = LocalSize.X / (InMaxX - InMinX);
...
FReply SSpline2DEditPanel::ZoomToFitClicked()
{
	ZoomToFit(true, true);
	return FReply::Handled();
}

TSharedRef<SWidget> SSpline2DEditPanel::GetPanelHeightMenu()
{
	TSharedRef<SWidget> FillWidget = SNew(SBorder)
		...
		[
			...
			[
				...
				[
					SNew(SSlider)
					.Value(this, &SSpline2DEditPanel::GetPanelHeightSliderPosition)
					.OnValueChanged(this, &SSpline2DEditPanel::OnSetPanelHeight)
				]
				+ SHorizontalBox::Slot()
				.AutoWidth()
				...
				[
					SNew(STextBlock)
					.Text(this, &SSpline2DEditPanel::GetPanelHeightLabel)
					.Font(FAppStyle::Get().GetFontStyle(TEXT("MenuItem.Font")))
				]
			]
...
FText SSpline2DEditPanel::GetPanelHeightLabel() const
{
	...
	TSharedRef<SWidget> FillWidget = SNew(SBorder)
		...
		[
			...
			[
				...
				[
					SNew(SSlider)
					.Value(this, &SSpline2DEditPanel::GetSplineThicknessSliderPosition)
					.OnValueChanged(this, &SSpline2DEditPanel::OnSetSplineThickness)
				]
				+ SHorizontalBox::Slot()
				.AutoWidth()
				.Padding(8.0f, 2.0f, 0.0f, 2.0f)
				[
					SNew(STextBlock)
					.Text(this, &SSpline2DEditPanel::GetSplineThicknessLabel)
					.Font(FAppStyle::Get().GetFontStyle(TEXT("MenuItem.Font")))
				]
			]
...
FText SSpline2DEditPanel::GetSplineThicknessLabel() const
{
	FNumberFormattingOptions Options;
	Options.SetMaximumFractionalDigits(0);
...
float SSpline2DEditPanel::GetSplineThicknessSliderPosition() const
{
	float Value = (SplineInfo.Get().Thickness - CONST_SplineThicknessMin) / (CONST_SplineThicknessMax - CONST_SplineThicknessMin);
	return Value;
}

void SSpline2DEditPanel::OnSetSplineThickness(float NewValue)
{
...
FText SSpline2DEditPanel::GetHelpText() const
{
	FText HelpText = LOCTEXT("SplineEditHelpText", "\n\
		...
		Mouse wheel: Zoom the editor view. Before zooming you need to click on the edit panel to focus on it.\n\
		Left mouse drag: Adjust spline point location, or adjust spline tangent direction and strength.\n\
		Right mouse drag: Pan the editor view."
	);
