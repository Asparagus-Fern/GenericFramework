...
{
	DisplayName = LOCTEXT("DisplayName", "Level Streaming Manager");
	ProcedureOrder = 0;
}
...
void UWorldManager::LoadLevel(TSoftObjectPtr<UWorld> Level, const bool bMakeVisibleAfterLoad, const bool bShouldBlockOnLoad, FOnFinish OnFinish)
{
	/* Check */
	if (!CheckLevel(Level))
	{
		DEBUG_LOG(Debug_World, Warning, TEXT("InValid Level"))
		return;
	}
	...
	{
		DEBUG_LOG(Debug_World, Warning, TEXT("Level Streaming Is Not Found"));
		return;
	}
	...
	if (LevelStreaming->IsLevelLoaded())
	{
		...
		DEBUG_LOG(Debug_World, Warning, TEXT("Level Is Already Load"));
		return;
	}
...
void UWorldManager::LoadLevels(TArray<TSoftObjectPtr<UWorld>> Levels, const bool bMakeVisibleAfterLoad, const bool bShouldBlockOnLoad, FOnOnceFinish OnOnceFinish, FOnFinish OnFinish)
{
	/* Make Sure Levels Is Not Empty */
	if (Levels.IsEmpty())
	{
		DEBUG_LOG(Debug_World, Warning, TEXT("Levels Is Empty"))
		OnFinish.ExecuteIfBound();
		return;
	}
...
void UWorldManager::LoadLevelsBySetting(TArray<FLoadLevelStreamingSetting> LoadLevelStreamingSettings, FOnOnceFinish OnOnceFinish, FOnFinish OnFinish)
{
	/* Make Sure Settings Is Not Empty */
	if (LoadLevelStreamingSettings.IsEmpty())
	{
		DEBUG_LOG(Debug_World, Warning, TEXT("Settings Is Empty"))
		OnFinish.ExecuteIfBound();
		return;
	}
...
void UWorldManager::UnloadLevel(TSoftObjectPtr<UWorld> Level, const bool bShouldBlockOnUnload, FOnFinish OnFinish)
{
	/* Check */
	if (!CheckLevel(Level))
	{
...
void UWorldManager::UnloadLevels(TArray<TSoftObjectPtr<UWorld>> Levels, const bool bShouldBlockOnUnload, FOnOnceFinish OnOnceFinish, FOnFinish OnFinish)
{
	/* Make Sure Levels Is Not Empty */
	if (Levels.IsEmpty())
	{
...
void UWorldManager::UnloadLevelsBySetting(TArray<FUnloadLevelStreamingSetting> UnloadLevelStreamingSettings, FOnOnceFinish OnOnceFinish, FOnFinish OnFinish)
{
	/* Make Sure Levels Is Not Empty */
	if (UnloadLevelStreamingSettings.IsEmpty())
	{
	...
	{
		if (UnloadLevelStreamingSetting.Level.IsNull()) { DEBUG_LOG(Debug_World, Warning, TEXT("InValid Level")) }
...
void UWorldManager::SetLevelVisibility(TSoftObjectPtr<UWorld> Level, bool bVisible, FOnFinish OnFinish)
{
	ULevelStreaming* LevelStreaming = GetLevelStreaming(Level);
...
void UWorldManager::SetLevelsVisibility(TArray<TSoftObjectPtr<UWorld>> Levels, bool bVisible, FOnOnceFinish OnOnceFinish, FOnFinish OnFinish)
{
	if (Levels.IsEmpty())
	{
		DEBUG_LOG(Debug_World, Warning, TEXT("Levels Is Empty"))
...
void UWorldManager::LoadCurrentWorldLevelStreaming(FOnOnceFinish OnOnceFinish, FOnFinish OnFinish)
{
	/* 获取需要加载的关卡 */
...
void UWorldManager::UnLoadCurrentWorldLevelStreaming(FOnOnceFinish OnOnceFinish, FOnFinish OnFinish)
{
	/* 获取需要卸载的流关卡 */
...
bool UWorldManager::IsCurrentWorldContainLevel(const TSoftObjectPtr<UWorld>& Level) const
{
	for (const auto StreamingLevel : GetWorld()->GetStreamingLevels())
	{
		FString PackageName;
...
bool UWorldManager::CheckLevel(TSoftObjectPtr<UWorld> Level)
{
	if (Level.IsNull())
	{
		DEBUG_LOG(Debug_World, Warning, TEXT("InValid Level"))
	...
	{
		DEBUG_LOG(Debug_World, Warning, TEXT("Current World Not Contain Level : %s"), *Level.ToString());
		return false;
	}
...
ULevelStreaming* UWorldManager::GetLevelStreaming(TSoftObjectPtr<UWorld> Level)
{
	if (!CheckLevel(Level))
	{
		return nullptr;
	...
	{
		DEBUG_LOG(Debug_World, Warning, TEXT("Level Is Not Found"))
		return nullptr;
	}
